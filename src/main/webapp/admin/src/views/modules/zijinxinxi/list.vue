		...
		<div v-if="showFlag">
			<el-form :inline="true" :model="searchForm" class="form-content">
				<el-row class="ad"
					...
					<el-form-item>
						<el-button v-if="isAuth('zijinxinxi','新增')" type="success" icon="el-icon-plus"
							@click="addOrUpdateHandler()">新增</el-button>
						&nbsp;
						<el-button v-if="isAuth('zijinxinxi','删除')" :disabled="dataListSelections.length <= 0"
							type="danger" icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
						&nbsp;
						<el-button v-if="isAuth('zijinxinxi','报表')" type="success" icon="el-icon-pie-chart"
							@click="chartDialog()">报表</el-button>
						&nbsp;
						<a style="text-decoration:none" class="el-button el-button--success"
							v-if="isAuth('zijinxinxi','导入导出')" icon="el-icon-download"
							href="http://localhost:8080/ertongyingloupaishe/upload/zijinxinxiMuBan.xls">批量导入资金信息数据模板</a>
						&nbsp;
						<el-upload v-if="isAuth('zijinxinxi','导入导出')" style="display: inline-block"
							action="ertongyingloupaishe/file/upload" :on-success="zijinxinxiUploadSuccess"
							:on-error="zijinxinxiUploadError" :show-file-list=false>
							<el-button v-if="isAuth('zijinxinxi','导入导出')" type="success" icon="el-icon-upload2">
								批量导入资金信息数据</el-button>
						</el-upload>
						&nbsp;
						<!-- 导出excel -->
						<download-excel v-if="isAuth('zijinxinxi','导入导出')" style="display: inline-block"
							class="export-excel-wrapper" :data="dataList" :fields="json_fields" name="zijinxinxi.xls">
							<!-- 导出excel -->
			...
			<div class="table-content">
				<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					v-if="isAuth('zijinxinxi','查看')" :data="dataList" v-loading="dataListLoading"
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="zijinxinxiName" header-align="center" label="标题">
						<template slot-scope="scope">
							{{scope.row.zijinxinxiName}}
						</template>
					</el-table-column>

					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="zijinxinxiNumber" header-align="center" label="相关金额">
						<template slot-scope="scope">
							{{scope.row.zijinxinxiNumber}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="zijinxinxiTypes" header-align="center" label="类型">
						<template slot-scope="scope">
							{{scope.row.zijinxinxiValue}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="zijinxinxiText" header-align="center" label="备注">
						<template slot-scope="scope">
							<span v-if="scope.row.zijinxinxiText != null &&scope.row.zijinxinxiText.length>10">
								{{scope.row.zijinxinxiText.slice(0,10)}}...
							</span>
							<span v-else>
								{{scope.row.zijinxinxiText}}
							</span>
						</template>
					</el-table-column>

					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="insertTime"
						header-align="center" label="插入时间">
						<template slot-scope="scope">
					...
					<el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
						<template slot-scope="scope">
							<el-button v-if="isAuth('zijinxinxi','查看')" type="success" icon="el-icon-tickets"
								size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button v-if="isAuth('zijinxinxi','修改')" type="primary" icon="el-icon-edit" size="mini"
								@click="addOrUpdateHandler(scope.row.id)">修改</el-button>

							<el-button v-if="isAuth('zijinxinxi','删除')" type="danger" icon="el-icon-delete" size="mini"
		...
		data() {
			return {
				...
				form: {
					id: null,
					zijinxinxiName: null,
					zijinxinxiNumber: null,
					zijinxinxiTypes: null,
					zijinxinxiText: null,
					insertTime: null,
					createTime: null,
				},
		...
		methods: {
			chartDialog() {
				...
				let params = {
					...
					thisTable: { //当前表
						tableName: 'zijinxinxi', //当前表表名,
						sumColum: 'zijinxinxi_number', //求和字段
						// date : 'insert_time',//分组日期字段
						// string : 'zijinxinxi_name',//分组字符串字段
						types: 'zijinxinxi_types', //分组下拉框字段
					},
					// joinTable : {//级联表（可以不存在）
			...
			getDataList() {
				...
				params['zijinxinxiDelete'] = 1 // 逻辑删除字段 1 未删除 2 删除
			...
			// 导入功能上传文件成功后调用导入方法
			zijinxinxiUploadSuccess(data) {
				let _this = this;
				_this.$http({
					url: "zijinxinxi/batchInsert?fileName=" + data.file,
					method: "get"
				}).then(({
					data
				}) => {
